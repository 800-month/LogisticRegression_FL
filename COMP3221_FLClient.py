...
# 改动
init_data = [client_id, sample_number, feature_number, '10.16.28.204', 6001]

# Create a UDP socket for sending global model / data size / ID to the server
sender_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
addr = (IP, port_server)
# sender_socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, ord(client_id[-1]))
sender_socket.connect(addr)
sender_socket.send(pickle.dumps(init_data))

t = 1
while True:
    print(t)
    data = sender_socket.recv(65507)
    global_W = pickle.loads(data)
    print(global_W)
    if global_W == 'finish':
        print('训练结束啦')
        break
    ...
    sender_socket.send(pickle.dumps(local_W_with_id))
    t += 1
